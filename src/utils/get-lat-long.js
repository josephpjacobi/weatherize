...
export async function getLatLong(address) {
  const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${address}&key=AIzaSyAlJFyBn5DLTB9ehxQn2J61pyvAZT5dk8o`;
  const coordinates = await fetch(url).then((response) => {
    ...
    .then((locationObject) => {
      ...
      address.location = locationObject.results[0].formatted_address;
      return address;
    })
    .catch((error) => {
      console.log(error)
